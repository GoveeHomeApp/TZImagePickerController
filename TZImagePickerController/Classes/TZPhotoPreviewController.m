...
- (void)select:(UIButton *)selectButton refreshCount:(BOOL)refreshCount {
    ...
    if (!selectButton.isSelected) {
        if (model.type == TZAssetModelMediaTypeVideo) {
            NSInteger videoTime = [model getVideoTime];
            if (videoTime > _tzImagePickerVc.allowVideoMaximumDuration) {
                [_tzImagePickerVc showAlertWithTitle:[NSBundle tz_localizedStringForKey:@"AllowVideoMaximumDuration"]];
                return;
            }
            if (videoTime < _tzImagePickerVc.allowVideoMinimumDuration) {
                [_tzImagePickerVc showAlertWithTitle:[NSBundle tz_localizedStringForKey:@"VideoTooShort"]];
                return;
            }
        }
